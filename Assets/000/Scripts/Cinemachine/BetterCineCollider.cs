...
namespace Cinemachine
{
    ...
    public class BetterCineCollider : CinemachineExtension
    {
        ...
        [TagField]
        [Tooltip("Obstacles with this tag will be allowed.  It is a good idea to set this field to the target's tag")]
        public List<string> m_AllowTags = new();
        ...
        {
            CustomRuntimeUtil.DestroyScratchCollider();
            base.OnDestroy();
        }
            ...
            Vector3 cameraPos, Vector3 lookAtPos, int layerMask, ref RaycastHit hitInfo)
        {
            ...
            if (targetDistance > Epsilon)
            {
                ...
                else
                {
                    ...
                    {
                        if (CustomRuntimeUtil.RaycastAllowTags(
                            ray, out hitInfo, rayLength, layerMask, m_AllowTags, gameObject.scene.GetPhysicsScene()))
                        {
            ...
            ref VcamExtraState extra)
        {
            ...
            if (CustomRuntimeUtil.RaycastAllowTags(ray, out var hitInfo, distance,
                    m_CollideAgainst & ~m_TransparentLayers, m_AllowTags, gameObject.scene.GetPhysicsScene()))
            {
            ...
            if (d < Epsilon || CustomRuntimeUtil.RaycastAllowTags(
            ...
            if (distance > Epsilon)
            {
                if (!CustomRuntimeUtil.RaycastAllowTags(ray, out hitInfo, distance,
                        m_CollideAgainst & ~m_TransparentLayers, m_AllowTags, gameObject.scene.GetPhysicsScene()))
                {
            ...
            Vector3 pos, Vector3 pushDir, RaycastHit obstacle, ref Vector3 outDir)
        {
            ...
            int numFound = physicsScene.SphereCast(
            ...
            if (numFound > 1)
            {
                ...
                for (int i = 0; i < numFound; ++i)
                {
                    if (m_CornerBuffer[i].collider == null)
                        continue;
                    bool evaluate = false;
                    foreach (var allowedTag in m_AllowTags)
                    {
                        if(allowedTag.Length > 0 && m_CornerBuffer[i].collider.CompareTag(allowedTag))
                        {
                            evaluate = true;
                            break;
                        }
        ...
        Vector3 RespectCameraRadius(Vector3 cameraPos, Vector3 lookAtPos)
        {
            ...
            RaycastHit hitInfo;
            PhysicsScene physicsScene = gameObject.scene.GetPhysicsScene();
            int numObstacles = physicsScene.OverlapSphere(
                ...
                && distance > m_MinimumDistanceFromTarget + Epsilon)
            {
                ...
                if (CustomRuntimeUtil.RaycastAllowTags(new Ray(targetPos, dir), 
                    out hitInfo, d, m_CollideAgainst, m_AllowTags, gameObject.scene.GetPhysicsScene()))
                {
            ...
            {
                var scratchCollider = CustomRuntimeUtil.GetScratchCollider();
                ...
                for (int i = 0; i < numObstacles; ++i)
                {
                    ...
                    foreach (var allowedTag in m_AllowTags)
                    {
                        if (allowedTag.Length > 0 && c.CompareTag(allowedTag))
                        {
                            evaluate = true;
                            break;
                        }
        ...
        bool CheckForTargetObstructions(CameraState state)
        {
            if (state.HasLookAt)
            {
                ...
                if (CustomRuntimeUtil.RaycastAllowTags(ray, out _,
