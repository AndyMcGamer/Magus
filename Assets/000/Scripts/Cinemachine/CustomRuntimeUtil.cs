...
namespace Cinemachine
{
    ...
    public static class CustomRuntimeUtil
    {
        ...
        /// <summary>
        /// Perform a raycast, but only allows objects that have one of the tags in the list
        /// </summary>
        ...
        /// <param name="allowedTags">Allowed tags</param>
        /// <returns>True if something was hit.  Results in hitInfo</returns>
        public static bool RaycastAllowTags(
            Ray ray, out RaycastHit hitInfo, float rayLength, int layerMask, in List<string> allowedTags, PhysicsScene physicsScene)
        {
            if (allowedTags.Count == 0)
            {
                if(physicsScene.Raycast(ray.origin, ray.direction, out hitInfo, rayLength, layerMask, QueryTriggerInteraction.Ignore))
                {
                    return true;
                }
            }
            ...
            {
                int closestHit = -1;
                int numHits = physicsScene.Raycast(
                    ray.origin, ray.direction, s_HitBuffer, rayLength, layerMask, QueryTriggerInteraction.Ignore);
                for (int i = 0; i < numHits; ++i)
                {
                    bool hasIgnoredTag = false;

                    for (int j = 0; j < allowedTags.Count; j++)
                    {
                        if (!s_HitBuffer[i].collider.CompareTag(allowedTags[j]))
                            hasIgnoredTag = true;
                    }
